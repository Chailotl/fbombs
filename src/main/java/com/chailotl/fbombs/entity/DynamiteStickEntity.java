...
public class DynamiteStickEntity extends ThrownItemEntity {
    ...
    private static final double BOUNCE_DAMPENER_HORIZONTAL = 0.4;
    ...
    public void tick() {
        ...
        if (this.getTick() <= 0 || this.isOnFire()) {
            if (explode(serverWorld, this.getBlockPos())) return;
        }
    ...
    protected void onBlockHit(BlockHitResult blockHitResult) {
        ...
        if ( getWorld() instanceof ServerWorld serverWorld) {
            if (getBounces() == 0) {
                if (explode(serverWorld, this.getBlockPos().offset(blockHitResult.getSide().getOpposite()))) return;
            } else if (getBounces() > 0) decrementBounces();
